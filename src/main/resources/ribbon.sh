...
JAVA_OPTS="-Duser.timezone=GMT+8 -server -Xms64m -Xmx64m -Xloggc:/app/code/CucPayTradePortalLog/gc.log"
...
APP_MAIN=org.ribbon.rzt.RztServer

#classpath参数,包括指定lib目录下的所有jar
TEMP_PATH=$CLASSPATH

for tradePortalJar in "$LIBS_PATH"/*.jar
do
   CLASSPATH="$TEMP_PATH":"$tradePortalJar"
done
